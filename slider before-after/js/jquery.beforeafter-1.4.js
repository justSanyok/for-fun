...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><p id="lt-arrow'+randID+'">BEFORE</p><div id="drag'+randID+'"></div><p id="rt-arrow'+randID+'">AFTER</p></div>'); // Create drag handle
			...
			$('#drag'+randID).width(2).height(imgHeight+20).css({'background':o.dividerColor,'position':'absolute'});	// Set drag handle CSS properties
			$('#lt-arrow'+randID).css({'color':'#fff','position':'absolute','opacity':1,'right':'12px','top':'7px','font': '17px sans-serif'});	// Set #lt-arrow
			$('#rt-arrow'+randID).css({'color':'#fff','position':'absolute','opacity':1,'left':'12px','top':'7px','font': '17px sans-serif'});	// Set #rt-arrow
			...
			function drag()
			{
				$('#lt-arrow'+randID+', #rt-arrow'+randID).stop().css('opacity',1);
			...
			function clickit()
			{
				...
				$(obj).click(function(e){
					...
					$('#lt-arrow'+randID+',#rt-arrow'+randID).stop().animate({opacity:1},50);
				});
